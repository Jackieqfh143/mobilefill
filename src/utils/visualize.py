...
def show_im2_html(web_title,web_header,web_dir,img_dir,im_size=256,sample_num = 3,max_num = 200):
    ...
    selected_imgs_idx = [random.randint(0,len(imgs) // (sample_num + 3) - 1) for i in range(max_num)]
    ...
    for idx in selected_imgs_idx:
        ...
        for i in range(sample_num):
            comp_name = f'{idx:0>5d}_im_out_{i}{img_suffex}'
