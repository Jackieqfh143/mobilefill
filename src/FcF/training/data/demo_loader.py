...
class ImageDataset(Dataset):
    ...
    def __init__(self,
        ...
        **super_kwargs,             # Additional arguments for the Dataset base class.
        ...
        self._all_mask_fnames = [os.path.relpath(os.path.join(root, fname), start=self.mask_path) for root, _dirs, files in
                            os.walk(self.mask_path) for fname in files]
        PIL.Image.init()
        self._image_mask_fnames = sorted(os.path.join(self.mask_path, fname) for fname in self._all_mask_fnames if
                                    self._file_ext(fname) in PIL.Image.EXTENSION)
        if len(self._image_mask_fnames) == 0:
            raise IOError('No image files found in the specified path')

        self.mask_data = sorted(self._image_mask_fnames)

        # for f in self._image_mask_fnames:
        #     if not '_mask' in f:
        #         self.files.append(f)
...
def get_loader(img_path,mask_path, resolution):
    """Returns torch.utils.data.DataLoader for custom coco dataset."""

    ds = ImageDataset(img_path=img_path,mask_path=mask_path, resolution=resolution)
