...
def visualize_demo(i, img, inv_mask, erased_img, pred_img, comp_img,save_path):
    lo, hi = [-1, 1]
    ...
    plt.imsave(save_path + f'/{i:0>5d}_im_real.jpg', img / 255)
    plt.imsave(save_path + f'/{i:0>5d}_im_masked.jpg', erased_img / 255)
    plt.imsave(save_path + f'/{i:0>5d}_im_out.jpg', comp_img / 255)
    plt.close()
...
@click.option('--mask_data', help='Training images (directory)', metavar='PATH', required=True)
@click.option('--save_path', help='Training images (directory)', metavar='PATH', required=True)
...
def generate_images(
    ...
    G = G.eval().to(device)
    # save_path = save_path + '/FCF'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    dataloader = get_loader(img_path=img_data, mask_path=mask_data,resolution=resolution)
    ...
    for i, data in tqdm(enumerate(dataloader, 0), total=len(dataloader), desc='Visualizing..'):
        ...
        with torch.no_grad():
            ...
            #     j += 1
            visualize_demo(i+1, images, invisible_masks, erased_images, pred_img.detach(), comp_img.detach(),save_path=save_path)
