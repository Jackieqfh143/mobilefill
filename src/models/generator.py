...
from src.modules.mobile_synthesis_block import MobileSynthesisBlock,MobileSynthesisBlock_withFFC,\
    MobileSynthesisBlock_v2,MobileSynthesisBlock_v3,MobileSynthesisBlock_v4,MobileSynthesisBlock_v5,MobileSynthesisBlock_v6
...
class MobileSynthesisNetwork_v6(nn.Module):
    def __init__(
            self,
            style_dim,
            channels = [512, 512, 512, 512, 256, 128, 64],
...
class MobileSynthesisNetwork_v7(nn.Module):
    def __init__(self,
                 style_dim = 512,
                 channels=[512, 512, 256, 256, 256, 128, 64],
                 blur_kernel=[1, 3, 3, 1],
                 device='cuda',
                 ):
        super(MobileSynthesisNetwork_v7, self).__init__()
        self.device = device
        self.input = ConstantInput(channels[0])
        self.conv1 = StyledConv(
            channels[0], channels[0], 3, style_dim, blur_kernel=blur_kernel
    ...
    def forward(self, style):
        ...
        _noise = torch.randn(1, 1, hidden.size(-2), hidden.size(-1)).to(style.device)
        hidden = self.conv1(hidden, style[:,0], noise=_noise)
        ...
        for i, m in enumerate(self.layers):
            ...
            shape = [2, 1, 1, 2 ** (i + 3), 2 ** (i + 3)]
            _noise = torch.randn(*shape).to(style.device)
            hidden, freq = m(hidden, style[:, start_style_idx:end_style_idx, :],
                             noise=_noise)
            idx += 2

        out = self.dwt_to_img(freq)
        return out
...
if __name__ == '__main__':
    device = "cuda"
    x = torch.randn(1,16,512).to(device)
    model = MobileSynthesisNetwork_v7(style_dim=512).to(device)
    print_network_params(model,"model")
    flop_counter(model, x)
    # summary(model,(16, 256,))
