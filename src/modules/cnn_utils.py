...
class CDilated_v2(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, d=1, groups=1):
        ...
        super().__init__()
        # padding = int((kSize - 1) / 2) * d
        pad_dic = {1:2,2:4,4:8,8:16}
        assert kSize == 5 and pad_dic.get(d)
        self.conv = nn.Conv2d(nIn, nOut,kSize, stride=stride, padding=pad_dic[d], bias=False,
                              dilation=d, groups=groups)
